    ...
    if (!filter_var($newemail, FILTER_VALIDATE_EMAIL) == true) {
        ...
        if (isset($_POST['newuser']) && !empty(str_replace(' ', '', $_POST['newuser'])) && $pwresp['status'] == 1) {
            ...
            if ($response == 1) {
                ...
                echo '<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'. $conf['signup_thanks'] .'</div><div id="returnVal" style="display:none;">true</div><div id="returnVal" style="display:none;">true</div>';
